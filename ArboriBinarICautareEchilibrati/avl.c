...
void inserareAvl(Nod** radacina, Prajitura p) {

	if ((*radacina)) {


		if ((*radacina)->info.ID < p.ID) {
			inserareAvl(&(*radacina)->right, p);
		}
		else
			if ((*radacina)->info.ID > p.ID) {
				inserareAvl(&(*radacina)->left, p);
			}


		int gradEch = gradEchilibru((*radacina));

		if (gradEch == 2) { //prea multe in stanga

			int grNouStang = gradEchilibru((*radacina)->left);

			if (grNouStang == 1) {
				RotireDreapta(&(*radacina));
			}
			else if (grNouStang == -1)
			{
				RotireStanga(&(*radacina)->left);
				RotireDreapta(&(*radacina)->left);
			}
		}
		else
			if (gradEch == -2) {  //prea multe in dreapta
				int grNouDrept = gradEchilibru((*radacina)->right);
...
int main() {
	Nod* radacina = NULL;
	inserareAvl(&radacina, crearePrajitura(10, "P10", 10));
	inserareAvl(&radacina, crearePrajitura(7, "P7", 7));
	inserareAvl(&radacina, crearePrajitura(3, "P4", 3));
	inserareAvl(&radacina, crearePrajitura(4, "P4", 4));
	inserareAvl(&radacina, crearePrajitura(11, "P11", 11));
	AfisarePRE(radacina);
